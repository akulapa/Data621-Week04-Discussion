...
AdRevDf <- read.csv("D:\\CUNY\\621\\Week04\\AdRevenue.csv", header= TRUE, stringsAsFactors = F)
...
p3 <- ggplot(data=AdRevDf, aes(Circulation,AdRevenue)) + 
...
- With outliers and without outliers, linear model resulted in `p-value` for $H_0: \beta_1 = 0$ less than `0.0001`. We can say with 95% confidence that `AdRevenue` is function `Circulation` and both are related.
...
In summary, data points are not `outliers`, but `highly influential leverage points`.
